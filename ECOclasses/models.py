...
class MajorTech(models.Model):
    major = models.ForeignKey(
        ...
        related_name='ECO_tech' #전공에 해당하는 테크. 재무, 마케팅 등등
    )
...
class MajorTrack(models.Model): #트랙당 전공이수학점이 달라서 해결하기 위함
    track = models.ForeignKey(
        Track,
        on_delete=models.CASCADE,
        related_name='track_ECOtrack' #트랙에 해당하는 메이저트랙
    )
    major = models.ForeignKey(
        Major,
        on_delete=models.CASCADE,
        related_name='major_ECOtrack' #전공에 해당하는 메이저트랙
    )
...
class Lecture(models.Model):
    ...
    category20 = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
        related_name='category20_ECO_lecture', #카테고리 안에 렉처
        null=True
    )
    category21 = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
        related_name='category21_ECO_lecture', #카테고리 안에 렉처
        null=True
    )
    category22 = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
        related_name='category22_ECO_lecture', #카테고리 안에 렉처
        null=True
    )
    category23 = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
        related_name='category23_ECO_lecture', #카테고리 안에 렉처
        null=True
    )
    category24 = models.ForeignKey(
        Category,
        on_delete=models.CASCADE,
        related_name='category24_ECO_lecture', #카테고리 안에 렉처
        null=True
    )
    # category = models.ForeignKey(
    #     Category,
    ...
    tech = models.ForeignKey(    #재무 마케팅...
        MajorTech,
        on_delete=models.CASCADE,
        related_name='majortech_ECO_lecture', #전공테크안에 렉처
        null=True,
        blank=True
    )
